...
import { loadActiveCoopMatches, loadCoopMatchByPartyId, partyIdForUser, persistCoopMatch, } from './coopMatchStore.js';
import { checkReleaseResult, compactMatchView, recomputeTeamStress, resolveParallelWindow, } from './coopMatchEngine.js';
export function mountCoopRoutes(app, deps) {
    ...
    function schedulePersistMatch(match) {
        void persistCoopMatch(prisma, match);
    }
    function evictFinishedMatch(match) {
    ...
    function broadcastMatchFrame(matchId, frame) {
    ...
    async function ensurePartyMatch(partyId) {
        const cachedId = matchByPartyId.get(partyId);
        if (cachedId) {
            const hit = matches.get(cachedId);
            if (hit && hit.status !== 'finished')
                return hit;
        }
        for (const m of matches.values()) {
            if (m.partyId === partyId && m.status !== 'finished')
                return m;
        }
    ...
    app.post('/neon_v1/coop/match/action', (req, res) => {
        ...
        const payload = typeof body.payload === 'object' && body.payload !== null && !Array.isArray(body.payload)
            ? body.payload
            : {};
        const expectedSeq = typeof body.expectedSeq === 'number' && Number.isFinite(body.expectedSeq)
        ...
        if (match.shared.mode === 'parallel_window') {
            ...
            if (action === 'apply_admin_infra' ||
                ...
                const missionIntensityTier = Math.max(1, Math.min(4, Math.floor(Number(payload.missionIntensityTier ?? match.shared.missionIntensityTier ?? 1))));
...
//# sourceMappingURL=mountCoopRoutes.js.map