...
import { mergePlayerSheetFromPreset, serializeNriPlayer, } from './nriSessionHelpers.js';
...
function normalizeInscribedName(raw) {
...
function canInscribeItem(item) {
    if (item.inscriptionLocked === true)
        return false;
    if (typeof item.inscribedName === 'string' && item.inscribedName.trim())
        return false;
    if (Array.isArray(item.tags) && item.tags.includes('вписать_имя'))
        return true;
...
export function mountNriPlayerRoutes(app, ctx) {
    ...
    app.patch('/neon_v1/services/nri/:code/player/notes', async (req, res) => {
        ...
        if (!parsed.ok)
            return sendApiError(res, 400, 'NRI_NOTES_REQUIRED', parsed.message);
    ...
    app.post('/neon_v1/services/nri/:code/player', async (req, res) => {
        ...
        if (!parsed.ok)
            return sendApiError(res, 400, 'NRI_NAME_REQUIRED', parsed.message);
    ...
    app.patch('/neon_v1/services/nri/:code/players/:userId', async (req, res) => {
        ...
        if (!parsed.ok)
            return sendApiError(res, 400, 'NRI_PLAYER_PATCH_INVALID', parsed.message);
    ...
    app.patch('/neon_v1/services/nri/:code/player/items/:itemId/inscribe', async (req, res) => {
        ...
        const code = String(req.params.code ?? '').trim().toUpperCase();
        const itemId = String(req.params.itemId ?? '').trim();
        const inscribedName = normalizeInscribedName(req.body?.inscribedName);
        if (!itemId)
            return sendApiError(res, 400, 'NRI_ITEM_ID', 'Укажите предмет.');
        if (!inscribedName) {
        ...
        try {
            ...
            const baseName = typeof current.baseName === 'string' && current.baseName.trim() ? current.baseName : current.name;
            inv[idx] = {
                ...current,
                baseName,
                inscribedName,
    ...
    app.post('/neon_v1/services/nri/:code/players/:userId/items/grant', async (req, res) => {
        ...
        if (!parsed.ok)
            return sendApiError(res, 400, 'NRI_CATALOG_ID', parsed.message);
    ...
    app.get('/neon_v1/services/nri/:code/player/holo-tattoo/options', async (req, res) => {
        ...
        try {
            ...
            if (!player)
                return sendApiError(res, 404, 'NRI_PLAYER_NOT_FOUND', 'Персонаж не найден.');
            const sheet = player.sheet && typeof player.sheet === 'object' ? { ...player.sheet } : {};
            ...
            const factionRefs = factions.map((f) => ({
                id: f.id,
                kind: f.kind,
                name: f.name,
            }));
            const options = buildHoloTattooOptions({ originId: typeof sheet.originId === 'string' ? sheet.originId : undefined }, factionRefs);
    ...
    app.post('/neon_v1/services/nri/:code/player/holo-tattoo', async (req, res) => {
        ...
        try {
            ...
            if (!result.ok)
                return sendApiError(res, 400, 'NRI_HOLO_TATTOO_FAILED', result.reason);
            if (rejectIfInvalidSheetConditions(res, result.sheet, sendApiError))
                return;
            const updated = await prisma.nriPlayer.update({
                where: { id: player.id },
...
//# sourceMappingURL=nriPlayerRoutes.js.map