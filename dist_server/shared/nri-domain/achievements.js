...
export const NRI_CLASS_ACHIEVEMENTS = [
    {
        id: 'daimyo_iron_wall',
        classId: 'daimyo',
...
export const NRI_ACHIEVEMENTS = [...UNIVERSAL_ACHIEVEMENTS, ...NRI_CLASS_ACHIEVEMENTS];
export const NRI_ACHIEVEMENT_BY_ID = Object.fromEntries(NRI_ACHIEVEMENTS.map((a) => [a.id, a]));
export function emptyAchievementState() {
    return { unlocked: [], unlockedAt: {}, progress: {} };
}
export function parseAchievementState(raw) {
    if (!raw || typeof raw !== 'object')
        return emptyAchievementState();
    const o = raw;
    const unlockedRaw = Array.isArray(o.unlocked) ? o.unlocked : [];
    const unlocked = unlockedRaw.filter((id) => typeof id === 'string' && id in NRI_ACHIEVEMENT_BY_ID);
    const unlockedAt = {};
    if (o.unlockedAt && typeof o.unlockedAt === 'object') {
        for (const [k, v] of Object.entries(o.unlockedAt)) {
    ...
    if (o.progress && typeof o.progress === 'object') {
        ...
        if (Array.isArray(p.drugsUsed)) {
            progress.drugsUsed = p.drugsUsed.filter((x) => typeof x === 'string');
        }
        if (Array.isArray(p.zonesVisited)) {
            progress.zonesVisited = p.zonesVisited.filter((x) => typeof x === 'string');
        }
        if (Array.isArray(p.medConsumablesUsed)) {
            progress.medConsumablesUsed = p.medConsumablesUsed.filter((x) => typeof x === 'string');
        }
        if (Array.isArray(p.mercWeaponZones)) {
            progress.mercWeaponZones = p.mercWeaponZones.filter((x) => typeof x === 'string');
        }
        if (typeof p.equipWeapon === 'boolean')
            progress.equipWeapon = p.equipWeapon;
        if (typeof p.equipArmor === 'boolean')
            progress.equipArmor = p.equipArmor;
    }
    return { unlocked, unlockedAt, progress };
}
function unlock(state, id, at = Date.now()) {
...
function unlockIfClass(state, id, classId, at) {
    const def = NRI_ACHIEVEMENT_BY_ID[id];
    if (def.classId && def.classId !== classId)
        return state;
...
function isMedConsumable(catalogId, category) {
    if (catalogId === 'g_medkit')
        return true;
    return category === 'consumable' || category === 'gear';
}
/** Применить событие; вернуть новое состояние и только что разблокированные id. */
export function applyAchievementEvent(state, event, opts = {}) {
    let next = state;
    ...
    if (event.type === 'hp_updated') {
        if (event.hp === 1)
            next = unlock(next, 'survived_1hp', at);
    }
    ...
    if (event.type === 'item_used') {
        ...
        if (id.startsWith('drug_') || event.category === 'drug') {
            ...
            if (allUsed)
                next = unlock(next, 'stoned_all_drugs', at);
        }
        if (id === 'g_medkit') {
    ...
    if (event.type === 'item_equipped' && !event.equipped) {
        const slot = event.slot;
        if (slot === 'weapon')
            next = { ...next, progress: { ...next.progress, equipWeapon: false } };
        if (slot === 'armor')
            next = { ...next, progress: { ...next.progress, equipArmor: false } };
    }
    ...
    if (event.type === 'zone_visited') {
        ...
        if (zonesVisited.length >= 5)
            next = unlock(next, 'cartographer', at);
        if (zonesVisited.length >= 3)
            next = unlockIfClass(next, 'detective_trail', classId, at);
        if (event.weaponEquipped) {
            ...
            if (mercWeaponZones.length >= 3)
                next = unlockIfClass(next, 'merc_patrol', classId, at);
        }
    }
...
export function achievementsForClass(classId) {
    return NRI_ACHIEVEMENTS.filter((a) => !a.classId || a.classId === classId);
}
export function dossierFromSheet(sheet) {
