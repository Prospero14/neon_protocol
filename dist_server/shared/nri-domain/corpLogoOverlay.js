...
import { shapeCellOffsets, } from './districtMegaMerge.js';
/** Logo footprint shapes (no corners/cross — clean marquees). */
export const CORP_LOGO_SHAPES = ['1x1', '2x1', '1x2', '2x2', '3x3', '6x6'];
export function isCorpLogoShape(s) {
    return CORP_LOGO_SHAPES.includes(s);
}
export function encodeCorpLogoArtId(themeId, shape) {
    return `logo:${themeId}:${shape}`;
}
export function encodeCorpLogoCoverArtId(anchorZoneKey) {
    return `logo_cover:${anchorZoneKey}`;
}
export function parseCorpLogoArtId(artId) {
    if (!artId)
        return null;
    ...
    if (artId.startsWith('logo:')) {
        ...
        if (!themeId || !isCorpLogoShape(shapeRaw))
            return null;
        return { kind: 'anchor', themeId, shape: shapeRaw };
    }
    return null;
}
export function corpLogoCellOffsets(shape) {
    if (shape === '1x1')
        return [[0, 0]];
...
export function buildCorpLogoInfo(tiles) {
...
//# sourceMappingURL=corpLogoOverlay.js.map