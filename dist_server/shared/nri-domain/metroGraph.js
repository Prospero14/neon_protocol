...
export function metroZoneKey(stationId) {
    return `${METRO_ZONE_PREFIX}${stationId}`;
}
export function parseMetroStationId(zoneKey) {
    if (!zoneKey || !zoneKey.startsWith(METRO_ZONE_PREFIX))
        return null;
    const id = zoneKey.slice(METRO_ZONE_PREFIX.length).trim();
    return id || null;
}
export function distMapUnits(a, b) {
    const dx = a.x - b.x;
    const dy = a.y - b.y;
    return Math.sqrt(dx * dx + dy * dy);
}
/** Default ride duration from map distance (viewBox 240×165). */
export function defaultTravelSeconds(from, to) {
    const d = distMapUnits(from, to);
    return Math.max(8, Math.round(12 + d * 1.8));
}
export function edgeTravelSeconds(edge, stations) {
    if (edge.travelSeconds != null && edge.travelSeconds > 0)
        return edge.travelSeconds;
...
export function findMetroPath(fromStationId, toStationId, stations, edges) {
    ...
    const queue = [{ id: fromStationId, hops: [], total: 0 }];
    const best = new Map();
    ...
    if (!winner)
        return { ok: false, error: 'Нет маршрута между станциями.' };
    return { ok: true, hops: winner.hops, totalSeconds: winner.total };
}
export function neighborStationIds(stationId, edges) {
    const out = new Set();
    for (const e of edges) {
        if (e.fromStationId === stationId)
            out.add(e.toStationId);
        if (e.toStationId === stationId)
            out.add(e.fromStationId);
    }
    return [...out];
}
...
//# sourceMappingURL=metroGraph.js.map