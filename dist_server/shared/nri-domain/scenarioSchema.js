...
import { emptyScenarioLinks, parseScenarioLinks } from './scenarioLinks.js';
...
function asStringArray(raw, maxItems = 64, maxLen = 64) {
...
export function normalizeScenarioLinks(raw) {
    ...
    return {
        ...
        placeTitle: typeof base.placeTitle === 'string' ? base.placeTitle.trim().slice(0, 120) : undefined,
        zoneKey: typeof base.zoneKey === 'string' && base.zoneKey.trim() ? base.zoneKey.trim().slice(0, 64) : null,
        mapMarkerId: typeof base.mapMarkerId === 'string' && base.mapMarkerId.trim()
            ? base.mapMarkerId.trim().slice(0, 64)
            : null,
        lorePlaceId: typeof base.lorePlaceId === 'string' && base.lorePlaceId.trim()
            ? base.lorePlaceId.trim().slice(0, 64)
            : null,
    };
}
export function normalizeScenarioNodeInput(input, partial = false) {
    const out = {};
...
//# sourceMappingURL=scenarioSchema.js.map