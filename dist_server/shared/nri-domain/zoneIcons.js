...
export function defaultZoneIconId(zoneType, zoneKey) {
    if (zoneKey.startsWith('corp_') || zoneType === 'corp')
        return 'corp';
    if (zoneType === 'highway' || zoneType === 'overpass')
        return zoneType;
...
export function resolveZoneIconHref(iconId, zoneType, zoneKey) {
    if (!iconId) {
        const preset = PRESET_BY_ID.get(defaultZoneIconId(zoneType, zoneKey));
        return preset?.dataUrl ?? null;
    }
    if (iconId.startsWith('url:'))
        return iconId.slice(4).trim() || null;
    if (iconId.startsWith('data:'))
        return iconId;
    return PRESET_BY_ID.get(iconId)?.dataUrl ?? null;
}
export function normalizeZoneIconId(raw, zoneType, zoneKey) {
    if (typeof raw !== 'string' || !raw.trim())
        return defaultZoneIconId(zoneType, zoneKey);
    const v = raw.trim();
    if (v.startsWith('url:') || v.startsWith('data:'))
        return v.slice(0, 500);
    if (PRESET_BY_ID.has(v))
        return v;
    return defaultZoneIconId(zoneType, zoneKey);
}
/** Иконка по метке сущности (фракции, места без района). */
export function defaultEntityIconId(entityTag) {
    switch (entityTag) {
        case 'corp':
            return 'corp';
        case 'gang':
...
export function resolveEntityIconHref(iconId, entityTag) {
    if (iconId?.startsWith('url:'))
        return iconId.slice(4).trim() || null;
    if (iconId?.startsWith('data:'))
        return iconId;
    const preset = PRESET_BY_ID.get(iconId ?? defaultEntityIconId(entityTag));
    return preset?.dataUrl ?? null;
}
//# sourceMappingURL=zoneIcons.js.map