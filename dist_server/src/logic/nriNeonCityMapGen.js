...
export function getMegaClusters() {
    return MEGAS.map((m) => ({
...
function subdivideCorpoPlaza(zones) {
    const out = [];
...
export function generateNeonCityZones() {
    let zones = [...HIGHWAYS, ...buildMegaZones()];
...
export function validateZoneCoverage(zones) {
    const COLS = 24;
    const ROWS = 33;
...
export function megaFromZoneKey(zoneKey) {
    const mk = megaKeyFromZoneKey(zoneKey);
    return mk ? (DEFAULT_MEGA_LABELS[mk] ?? null) : null;
}
//# sourceMappingURL=nriNeonCityMapGen.js.map