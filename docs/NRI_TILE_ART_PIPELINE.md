...
- **Целевой вид района (north star):** [`refs/nri-district-target-look.jpg`](./refs/nri-district-target-look.jpg)
...
- Погода в редакторе: `NriDistrictWeatherFx` (дождь/вуаль), лампы: `NriMapLampLayer`

---

## Target look (обязательный ориентир)

Район в итоге должен читаться как изометрический cyberpunk street diorama в духе референса:

![Target district look](./refs/nri-district-target-look.jpg)

### Что копируем из рефа

| Слой | Требование |
|---|---|
| **Камера** | Фиксированная изометрия ~¾ сверху (как Shadowrun Returns / тактическая RPG), не top-down «плоская карта» |
| **Дороги** | Тёмный мокрый асфальт, чёткая разметка (пунктир / зебра / вафля на crossing), стыки клеток без швов |
| **Тротуары / бордюры** | Приподнятая кромка у зданий, решётки, лужи-блики от неона |
| **Здания** | Модульные фасады одного языка: металл/бетон, окна с тёплым светом, неон на вывесках |
| **Пропсы** | Лавки с навесами, ящики, фонари, редкие машины — одна плотность, не каша |
| **Свет** | Тёмный ambient + точечные неон/витрины (cyan / amber / green / red), мягкий bloom, без «фиолетового AI» |
| **Атмосфера** | Дождь (диагональные полосы), дымка/туман вдаль, мокрые отражения на асфальте |
| **Единый стиль** | Все placeType выглядят как один сет ассетов, не микс flat-SVG + фото + low-poly |

### Что уже есть в коде vs что добиваем артом

| Уже в рантайме | Добиваем ассетами / FX |
|---|---|
| Процедурные дороги + уголки + вафля crossing | PNG/SVG спрайты домов/лавок в стиле рефа |
| Кисти размеров, поворот, лого | Единая палитра и inset под изометрию |
| `Дождь` toggle → `NriDistrictWeatherFx` | Гуще дождь + лёгкий fog gradient по глубине |
| Ночной FX сейчас выключен (`NriDistrictNightFx` → null) | Вернуть мягкие окна/неон без пересвета |
| Лампы на карте | Согласовать цвет ламп с палитрой рефа |

### Дорожная карта (практическая)

1. **Style bible** (этот раздел + промпты) — done baseline.
2. **Пилотная серия 1×1** в одном стиле: `road`, `house`, `shop`, `shack`, `crossing` — сгенерить с `reference_image_paths` → реф, довести, drop-in.
3. **Стыкуемые дороги** 2×1 / уголки / crossing — те же материалы асфальта/разметки.
4. **Атмослой**: усилить weather + опционально fog overlay в `NriDistrictWeatherFx`.
5. **Ночь**: аккуратно включить `NriDistrictNightFx` (окна/неон), не как раньше «слишком ярко».
6. Массовая замена галереи `district-building-gallery.json` на новые спрайты.
