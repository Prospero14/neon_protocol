...
async function ensurePublicRoom(prisma: PrismaClient) {
...
async function findOrCreateDmRoom(prisma: PrismaClient, selfId: string, peerId: string) {
...
async function canAccessRoom(
  prisma: PrismaClient,
  room: { kind: string; slug: string | null; dmKey: string | null },
  userId: string,
  isAdmin: boolean
): Promise<boolean> {
  if (room.kind === 'public') return true;
...
function serializeMessage(msg: {
  ...
  return {
    ...
    npcArchetype:
      isNpc && typeof payload?.npcArchetype === 'string' ? payload.npcArchetype : undefined,
...
export function mountChatService(app: Express, deps: ChatServiceDeps) {
  ...
  app.get('/neon_v1/services/chat/rooms', async (req, res) => {
    ...
    if (!auth) return sendApiError(res, 401, 'CHAT_NO_TOKEN', 'Нет токена авторизации.');
    try {
  ...
  app.post('/neon_v1/services/chat/dm', async (req, res) => {
    ...
    try {
      ...
      if (!peer) return sendApiError(res, 404, 'CHAT_PEER_NOT_FOUND', 'Пользователь не найден.');
  ...
  app.get('/neon_v1/services/chat/rooms/:roomId/messages', async (req, res) => {
    ...
    try {
      ...
      if (!room) return sendApiError(res, 404, 'CHAT_ROOM_NOT_FOUND', 'Комната не найдена.');
  ...
  app.post('/neon_v1/services/chat/rooms/:roomId/messages', async (req, res) => {
    ...
    if (!auth) return sendApiError(res, 401, 'CHAT_NO_TOKEN', 'Нет токена авторизации.');
    const roomId = req.params.roomId;
    const { text, asNpcId, dmTargetUserId, nriCode } = req.body as {
      text?: string;
      asNpcId?: string;
    ...
    try {
      ...
      if (typeof asNpcId === 'string' && asNpcId.trim()) {
        ...
        if (!npc) return sendApiError(res, 404, 'CHAT_NPC_NOT_FOUND', 'НПС не найден.');
        const sheet =
  ...
  async function roomSpamBotEnabled(
    room: { id: string; kind: string; slug: string | null }
  ): Promise<boolean> {
  ...
  app.get('/neon_v1/services/chat/rooms/:roomId/participants', async (req, res) => {
    ...
    try {
      ...
      const byUser = new Map<string, { userId: string; username: string; isAdmin: boolean }>();
      for (const msg of recent) {
        if (msg.user.username === SPAM_BOT_USERNAME) continue;
      ...
      if (room.kind === 'nri' && room.slug) {
        ...
        if (session) {
          for (const p of participants) {
            if (p.userId === session.hostUserId) p.isHost = true;
          }
        }
      }
  ...
  app.post('/neon_v1/services/chat/rooms/:roomId/file', async (req, res) => {
    ...
    try {
      ...
      if (!file) return sendApiError(res, 404, 'CHAT_FILE_NOT_FOUND', 'Файл не найден.');
      const isAdmin = isAdminUsername(me.username);
