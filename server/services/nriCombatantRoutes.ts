...
export type NriCombatantRouteDeps = {
  ...
  resolveSession: (code: string) => Promise<{ id: string; hostUserId: string; status: string } | null>;
  ...
  parseJsonField: (raw: unknown) => unknown | null;
};
...
export function mountNriCombatantRoutes(app: Express, deps: NriCombatantRouteDeps): void {
