...
async function findOrCreateDmRoom(prisma: PrismaClient, userA: string, userB: string) {
...
function itemStatsLine(item: InvItem): string {
  const parts: string[] = [];
  const mods = item.c2185Mods as Record<string, number> | undefined;
  if (mods) {
  ...
  const atk = item.attack as { damageDice?: string; damageType?: string } | undefined;
  if (atk?.damageDice) parts.push(`${atk.damageDice} ${atk.damageType ?? ''}`.trim());
  return parts.join(' · ') || 'без боевых бонусов';
}

function transferText(fromName: string, toName: string, itemName: string): string {
  return `${fromName} передаёт ${toName}: «${itemName}».`.slice(0, 2000);
}

export function mountNriItemTransferRoutes(app: Express, deps: Deps) {
  ...
  app.post('/neon_v1/services/nri/:code/items/transfer', async (req, res) => {
    ...
    const { toUserId, itemId, fromNpcId, asNpcId, catalogId, qty } = req.body as {
      toUserId?: string;
      itemId?: string;
      fromNpcId?: string;
      asNpcId?: string;
    ...
    try {
      ...
      if (!me) return sendApiError(res, 401, 'NRI_USER', 'Пользователь не найден.');
      ...
      let item: InvItem | null = null;
      let fromDisplayName = me.username;
      let fromNpcIdOut: string | null = null;
      let fromNpcImage: string | null = null;
      let fromNpcArchetype: string | undefined;
      let senderInventory: InvItem[] | undefined;
      ...
      if (typeof catalogId === 'string' && catalogId.trim()) {
        ...
        if (!item) return sendApiError(res, 400, 'NRI_CATALOG_UNKNOWN', 'Неизвестный предмет.');
        ...
        const sheet =
          npc.sheet && typeof npc.sheet === 'object'
            ? (npc.sheet as { npcArchetype?: unknown })
            : null;
        if (typeof sheet?.npcArchetype === 'string') fromNpcArchetype = sheet.npcArchetype;
      } else {
        if (typeof itemId !== 'string' || !itemId.trim()) {
        ...
        const inv: InvItem[] = Array.isArray(sender.inventory) ? [...(sender.inventory as InvItem[])] : [];
      ...
      const npcForDisplay =
        typeof asNpcId === 'string' && asNpcId.trim()
      ...
      const recvInv: InvItem[] = Array.isArray(recipient.inventory)
        ? [...(recipient.inventory as InvItem[])]
        : [];
      const nextInv = mergeInventoryItem(recvInv, item);
      await prisma.nriPlayer.update({
        where: { id: recipient.id },
        data: { inventory: nextInv as object[] },
      });
  ...
  app.post('/neon_v1/services/nri/:code/items/transfer/broadcast', async (req, res) => {
    ...
    try {
      ...
      if (!msg?.payload) return sendApiError(res, 404, 'NRI_MSG_NOT_FOUND', 'Сообщение не найдено.');
      ...
      const item = payload.item as InvItem | undefined;
