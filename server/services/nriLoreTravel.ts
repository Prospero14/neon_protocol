...
function parseScenarioLinks(raw: unknown): ScenarioLinks {
...
function evaluateCheckpoint(
  node: { id: string; links: ScenarioLinks },
  currentScriptNodeId: string | null,
  playerZoneKeys: string[],
  playerCount: number
): { met: boolean } {
  const links = node.links;
  if (!links.meetCheckpoint || !links.zoneKey) return { met: false };
  if (currentScriptNodeId !== node.id) return { met: false };
  if (playerCount <= 0) return { met: false };
  const atPlace = playerZoneKeys.filter((z) => z === links.zoneKey).length;
  return { met: atPlace >= playerCount };
}

function zoneCenter(z: ZoneCenter) {
  return { x: z.x + z.w / 2, y: z.y + z.h / 2 };
}

function travelMinutes(from: ZoneCenter | null, to: ZoneCenter, opts: { vehicleSpeed?: number; onFoot?: boolean }) {
  ...
  if (opts.onFoot || !opts.vehicleSpeed) return Math.max(5, Math.round(dist * 0.35));
...
type Deps = {
  ...
  resolveSession: (
    code: string
  ) => Promise<{ id: string; hostUserId: string; chatRoomId: string; status: string } | null>;
  resolveUser: (auth: JwtAuth) => Promise<{ id: string; username: string } | null>;
  requireHost: (
...
async function sendServiceDmToHost(
  prisma: PrismaClient,
  hostUserId: string,
  fromUserId: string,
  text: string,
...
export async function ensureSessionLorePlacesFromMap(prisma: PrismaClient, sessionId: string) {
  ...
  const byZone = new Map(
    existing.filter((p) => p.zoneKey).map((p) => [p.zoneKey as string, p])
  );
  for (const zone of zones) {
    ...
    if (hit) {
      ...
      if (!hit.summary?.trim()) patch.summary = summary;
      if (!hit.iconId) patch.iconId = iconId;
      if (!hit.entityTag && entityTag) patch.entityTag = entityTag;
...
export async function ensureSessionFactionsFromCorpZones(prisma: PrismaClient, sessionId: string) {
  ...
  if (corpZones.length === 0) return;
  const factions = await prisma.nriFaction.findMany({ where: { sessionId } });
  const zoneTaken = new Set<string>();
  for (const f of factions) {
    for (const zk of parseZoneKeys(f.zoneKeys)) zoneTaken.add(zk);
  }
  for (const zone of corpZones) {
    if (zoneTaken.has(zone.zoneKey)) continue;
...
export async function syncLorePlacesFromZonePatch(
  prisma: PrismaClient,
  sessionId: string,
  zone: {
    zoneKey: string;
...
async function syncFactionZonePlaces(
  prisma: PrismaClient,
  sessionId: string,
  factionId: string,
  zoneKeys: string[],
  ...
  for (const zoneKey of zoneKeys) {
    ...
    if (!zone || zoneKey.startsWith('__')) continue;
...
const placePropagateLocks = new Map<string, Promise<void>>();

export async function propagatePlaceUpdate(
  prisma: PrismaClient,
  sessionId: string,
  place: {
    id: string;
  ...
  try {
    ...
    release();
    if (placePropagateLocks.get(sessionId) === gate) placePropagateLocks.delete(sessionId);
  }
}

async function propagatePlaceUpdateInner(
  prisma: PrismaClient,
  sessionId: string,
  place: {
    id: string;
  ...
  for (const node of nodes) {
    ...
    const linked =
      links.lorePlaceId === place.id || (place.zoneKey && links.zoneKey === place.zoneKey);
    if (!linked) continue;
    const nextLinks = { ...(node.links as Record<string, unknown>) };
    ...
    await prisma.nriScenarioNode.update({
      where: { id: node.id },
      data: { links: nextLinks as Prisma.InputJsonValue },
    });
  }
}

async function syncLorePlaceFromNode(
  prisma: PrismaClient,
  sessionId: string,
  nodeId: string,
  title: string,
  links: ReturnType<typeof parseScenarioLinks>
) {
  if (!links.syncToLore) return links.lorePlaceId ?? null;
...
export function mountNriLoreTravelRoutes(app: Express, deps: Deps) {
  ...
  app.post('/neon_v1/services/nri/:code/lore/entries', async (req, res) => {
    ...
    if (!auth) return sendApiError(res, 401, 'NRI_NO_TOKEN', 'Нет токена авторизации.');
    const code = String(req.params.code ?? '').trim().toUpperCase();
    const { title, body, summary } = req.body as { title?: string; body?: string; summary?: string };
    try {
      const session = await resolveSession(code);
      if (!session) return sendApiError(res, 404, 'NRI_NOT_FOUND', 'Стол не найден.');
      const me = await resolveUser(auth);
  ...
  app.patch('/neon_v1/services/nri/:code/lore/entries/:entryId', async (req, res) => {
    ...
    try {
      ...
      if (!existing) return sendApiError(res, 404, 'NRI_LORE_ENTRY_NOT_FOUND', 'Карточка не найдена.');
  ...
  app.post('/neon_v1/services/nri/:code/lore/factions', async (req, res) => {
    ...
    if (!auth) return sendApiError(res, 401, 'NRI_NO_TOKEN', 'Нет токена авторизации.');
    const code = String(req.params.code ?? '').trim().toUpperCase();
    const { name, description, summary, color, kind, zoneKeys, iconId } = req.body as {
      name?: string;
      description?: string;
      summary?: string;
      color?: string;
    ...
    try {
      ...
      await syncFactionZonePlaces(
        prisma,
        session.id,
        f.id,
        parseZoneKeys(f.zoneKeys),
        f.name,
        normalizeFactionKind(f.kind)
      );
  ...
  app.patch('/neon_v1/services/nri/:code/lore/factions/:factionId', async (req, res) => {
    ...
    const { name, description, summary, color, memberPlayerIds, memberNpcIds, kind, zoneKeys, iconId } = req.body as {
      name?: string;
      description?: string;
      summary?: string;
      color?: string | null;
    ...
    try {
      ...
      if (!existing) return sendApiError(res, 404, 'NRI_FACTION_NOT_FOUND', 'Фракция не найдена.');
      ...
      if (Array.isArray(zoneKeys) || kind !== undefined || typeof name === 'string') {
        await syncFactionZonePlaces(
          ...
          session.id,
          updated.id,
          parseZoneKeys(updated.zoneKeys),
          updated.name,
          normalizeFactionKind(updated.kind)
        );
      }
  ...
  app.patch('/neon_v1/services/nri/:code/lore/faction-relations', async (req, res) => {
    ...
    if (!auth) return sendApiError(res, 401, 'NRI_NO_TOKEN', 'Нет токена авторизации.');
    const code = String(req.params.code ?? '').trim().toUpperCase();
    const { enabled, edges } = req.body as { enabled?: boolean; edges?: Record<string, string> };
    try {
      const session = await resolveSession(code);
      if (!session) return sendApiError(res, 404, 'NRI_NOT_FOUND', 'Стол не найден.');
      const me = await resolveUser(auth);
      ...
      const prevMatrix = parseFactionRelationMatrix(
        prev ? { enabled: prev.enabled, edges: prev.edges } : null
      );
  ...
  app.patch('/neon_v1/services/nri/:code/lore/places/:placeId', async (req, res) => {
    ...
    const { title, body, summary, entityTag, iconId } = req.body as {
      title?: string;
      body?: string;
      summary?: string;
      entityTag?: string | null;
    ...
    try {
      ...
      if (!existing) return sendApiError(res, 404, 'NRI_PLACE_NOT_FOUND', 'Место не найдено.');
  ...
  app.post('/neon_v1/services/nri/:code/map/move', async (req, res) => {
    ...
    if (!auth) return sendApiError(res, 401, 'NRI_NO_TOKEN', 'Нет токена авторизации.');
    const code = String(req.params.code ?? '').trim().toUpperCase();
    const { zoneKey, vehicleId, overload } = req.body as {
      zoneKey?: string;
    ...
    try {
      ...
      if (!targetZone) return sendApiError(res, 404, 'NRI_ZONE_NOT_FOUND', 'Район не найден.');

      const prev = await prisma.nriPlayerPosition.findUnique({
        where: { sessionId_userId: { sessionId: session.id, userId: auth.userId } },
      });
      let fromZone: ZoneCenter | null = null;
      if (prev?.zoneKey) {
        const z = await prisma.nriMapZone.findUnique({ where: { zoneKey: prev.zoneKey } });
        if (z) fromZone = { zoneKey: z.zoneKey, x: z.x, y: z.y, w: z.w, h: z.h };
      }
      const toZone: ZoneCenter = {
      ...
      if (typeof vehicleId === 'string' && vehicleId.trim()) {
        ...
        if (!vehicle) return sendApiError(res, 404, 'NRI_VEHICLE_NOT_FOUND', 'Транспорт не найден.');
  ...
  return { syncLorePlaceFromNode, evaluateScenarioCheckpoints: async (sessionId: string) => {
...
export type NriLoreTravelHelpers = ReturnType<typeof mountNriLoreTravelRoutes>;
