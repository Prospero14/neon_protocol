...
import {
  listMapZones,
  ensureMapZonesSeeded,
  patchMapZone,
  createMapSubZone,
  createMapTopZone,
  deleteMapZone,
  regenerateDistrictTilesForParent,
  clearDistrictTiles,
} from './nriMapZones.js';
import {
...
export function mountNriMapRoutes(app: Express, ctx: NriRouteContext): void {
  ...
  app.patch('/neon_v1/services/nri/:code/map/zones/:zoneKey', async (req, res) => {
    ...
    const { name, corpName, pois, megaDistrict, color, iconId, artId, placeType, districtStyle, populationBand, densityLabel, trafficLevel, nightlifeLevel, rotation, swapWithZoneKey, x, y, w, h, zoneType } = req.body as {
      name?: string;
    ...
    try {
      ...
      if ('error' in result) {
        return sendApiError(
          ...
          400,
          String(result.error),
          result.error === 'NAME_REQUIRED' ? 'Укажите название района.' : 'Не удалось создать район.'
        );
      }
  ...
  app.delete('/neon_v1/services/nri/:code/map/zones/:zoneKey', async (req, res) => {
    ...
    const confirmName =
      typeof (req.body as { confirmName?: string })?.confirmName === 'string'
        ? (req.body as { confirmName: string }).confirmName
