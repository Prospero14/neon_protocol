...
export function mountNriPresetRoutes(app: Express, ctx: NriRouteContext): void {
  ...
  app.post('/neon_v1/services/nri/:code/presets', async (req, res) => {
    ...
    if (!parsed.ok) return sendApiError(res, 400, 'NRI_PRESET_LABEL', parsed.message);
    ...
    try {
      ...
      const preset = await prisma.nriPresetCharacter.create({
        data: {
          ...
          inventory: (Array.isArray(inventory) ? inventory : []) as object[],
  ...
  app.patch('/neon_v1/services/nri/:code/presets/:presetId', async (req, res) => {
    ...
    if (!parsed.ok) return sendApiError(res, 400, 'NRI_PRESET_PATCH_INVALID', parsed.message);
    const { label, classId, inventory, sheet, portraitUrl, sortOrder, publishedToPlayers } = parsed.data;
    try {
      const session = await resolveSession(code);
      if (!session) return sendApiError(res, 404, 'NRI_NOT_FOUND', 'Стол не найден.');
      ...
      const preset = await prisma.nriPresetCharacter.update({
        ...
        data: {
          ...
          ...(inventory !== undefined ? { inventory: (Array.isArray(inventory) ? inventory : []) as object[] } : {}),
