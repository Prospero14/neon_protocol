...
function mergePlayerSheetFromPreset(
  presetSheet: unknown,
  displayName: string,
  clientSheet?: unknown
...
export function mountNriService(app: Express, deps: NriServiceDeps) {
  ...
  app.post('/neon_v1/services/nri/:code/ice/score', async (req, res) => {
    ...
    if (!auth) return sendApiError(res, 401, 'NRI_NO_TOKEN', 'Нет токена авторизации.');
    const code = String(req.params.code ?? '').trim().toUpperCase();
    const { score, exfilPct, tracePct, won } = req.body as {
      score?: number;
      exfilPct?: number;
      tracePct?: number;
      won?: boolean;
  ...
  app.patch('/neon_v1/services/nri/:code/presets/:presetId', async (req, res) => {
    ...
    try {
      ...
      if (nextSheet !== undefined && rejectIfInvalidSheetConditions(res, nextSheet, sendApiError)) return;
