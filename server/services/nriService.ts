...
export function mountNriService(app: Express, deps: NriServiceDeps) {
  ...
  app.post('/neon_v1/services/nri/:code/wonlongs/transfer', async (req, res) => {
    ...
    if (!auth) return sendApiError(res, 401, 'NRI_NO_TOKEN', 'Нет токена авторизации.');
    const code = String(req.params.code ?? '').trim().toUpperCase();
    const { amount, toPlayerUserId, toNpcId, memo } = req.body as {
      amount?: number;
  ...
  app.post('/neon_v1/services/nri/:code/wonlongs/grant', async (req, res) => {
    ...
    if (!auth) return sendApiError(res, 401, 'NRI_NO_TOKEN', 'Нет токена авторизации.');
    const code = String(req.params.code ?? '').trim().toUpperCase();
    const { playerUserId, amount, fromNpcId, memo } = req.body as {
      playerUserId?: string;
      amount?: number;
      fromNpcId?: string;
      memo?: string;
  ...
  app.post('/neon_v1/services/nri/:code/player', async (req, res) => {
    ...
    if (!auth) return sendApiError(res, 401, 'NRI_NO_TOKEN', 'Нет токена авторизации.');
    const code = String(req.params.code ?? '').trim().toUpperCase();
    const { displayName, classId, presetId, sheet, inventory } = req.body as {
      displayName?: string;
      classId?: string;
      presetId?: string;
      sheet?: unknown;
  ...
  app.patch('/neon_v1/services/nri/:code/players/:userId', async (req, res) => {
    ...
    const { displayName, sheet } = req.body as { displayName?: string; sheet?: unknown };
    try {
      const session = await resolveSession(code);
      if (!session) return sendApiError(res, 404, 'NRI_NOT_FOUND', 'Стол не найден.');
      const me = await resolveUser(auth);
  ...
  app.patch('/neon_v1/services/nri/:code/map/zones/:zoneKey', async (req, res) => {
    ...
    const { name, corpName, pois, megaDistrict } = req.body as {
      name?: string;
    ...
    try {
      ...
      const zone = await patchMapZone(prisma, zoneKey, { name, corpName, pois, megaDistrict });
  ...
  app.post('/neon_v1/services/nri/:code/presets', async (req, res) => {
    ...
    if (!auth) return sendApiError(res, 401, 'NRI_NO_TOKEN', 'Нет токена авторизации.');
    const code = String(req.params.code ?? '').trim().toUpperCase();
    const { label, classId, inventory, sheet, portraitUrl, sortOrder, publishedToPlayers } = req.body as {
      label?: string;
      classId?: string;
      inventory?: unknown;
      sheet?: unknown;
    ...
    try {
      ...
      const preset = await prisma.nriPresetCharacter.create({
        data: {
          ...
          publishedToPlayers: publishedToPlayers === true,
        },
      });
    ...
    try {
      ...
      if (existing.claimedByUserId) {
        const changingStructure =
          (typeof classId === 'string' && classId.trim() && classId.trim() !== existing.classId) ||
          inventory !== undefined;
      ...
      if (existing.claimedByUserId && (nextSheet || (typeof label === 'string' && label.trim()))) {
        ...
        if (player) {
          const prevSheet =
            player.sheet && typeof player.sheet === 'object'
              ? ({ ...(player.sheet as object) } as Record<string, unknown>)
              : {};
          await prisma.nriPlayer.update({
            where: { id: player.id },
            data: {
  ...
  app.post('/neon_v1/services/nri/:code/players/:userId/items/grant', async (req, res) => {
    ...
    const code = String(req.params.code ?? '').trim().toUpperCase();
    const targetUserId = req.params.userId;
    const { catalogId, qty, fromNpcId } = req.body as {
      catalogId?: string;
      qty?: number;
      fromNpcId?: string;
    };
