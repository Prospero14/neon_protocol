...
import {
  ...
  type MetroEdge,
  type MetroStation,
} from '../../shared/nri-domain/metroGraph.js';
...
import { mergeInventoryItem, type InvItem } from './nriItemGrant.js';
...
import type { NriRouteContext } from './nriMapRoutes.js';
...
async function assertUnderhiveAccess(
  ...
  const hasPassport = inv.some(
    (raw) =>
      raw &&
      typeof raw === 'object' &&
      (raw as { catalogId?: unknown }).catalogId === 'g_underhive_passport'
  );
...
export function mountNriUnderhiveMetroRoutes(app: Express, ctx: NriRouteContext): void {
  ...
  app.post('/neon_v1/services/nri/:code/map/lamps', async (req, res) => {
    ...
    if (!auth) return sendApiError(res, 401, 'NRI_NO_TOKEN', 'Нет токена авторизации.');
    const code = String(req.params.code ?? '').trim().toUpperCase();
    const { x, y, color, on, radius, layer, parentZoneKey } = req.body as {
      x?: number;
      y?: number;
      color?: string;
      on?: boolean;
    ...
    try {
      ...
      const lamp = await prisma.nriMapLamp.create({
        data: {
          ...
          parentZoneKey:
            typeof parentZoneKey === 'string' && parentZoneKey.trim() ? parentZoneKey.trim() : null,
        },
      });
  ...
  app.patch('/neon_v1/services/nri/:code/map/lamps/:lampId', async (req, res) => {
    ...
    const { color, on, radius, x, y } = req.body as {
      color?: string;
      on?: boolean;
      radius?: number;
      x?: number;
    ...
    try {
      ...
      if (!existing) return sendApiError(res, 404, 'NRI_LAMP_NOT_FOUND', 'Фонарь не найден.');
  ...
  app.post('/neon_v1/services/nri/:code/map/metro/stations', async (req, res) => {
    ...
    if (!auth) return sendApiError(res, 401, 'NRI_NO_TOKEN', 'Нет токена авторизации.');
    const code = String(req.params.code ?? '').trim().toUpperCase();
    const { lineId, name, x, y, districtZoneKey, connectFromStationId, travelSeconds } = req.body as {
      lineId?: string;
      name?: string;
      x?: number;
      y?: number;
    ...
    try {
      ...
      if (!line) return sendApiError(res, 404, 'NRI_METRO_LINE_NOT_FOUND', 'Ветка не найдена.');
      const station = await prisma.nriMetroStation.create({
        data: {
          ...
          districtZoneKey:
            typeof districtZoneKey === 'string' && districtZoneKey.trim() ? districtZoneKey.trim() : null,
          extraLineIds: [],
        },
      });
      let edge = null;
        ...
        if (from) {
          edge = await prisma.nriMetroEdge.create({
            data: {
              ...
              travelSeconds:
                typeof travelSeconds === 'number' && travelSeconds > 0
  ...
  app.delete('/neon_v1/services/nri/:code/map/metro/stations/:stationId', async (req, res) => {
    ...
    try {
      ...
      if (!existing) return sendApiError(res, 404, 'NRI_METRO_STATION_NOT_FOUND', 'Станция не найдена.');
  ...
  app.post('/neon_v1/services/nri/:code/map/metro/edges', async (req, res) => {
    ...
    if (!auth) return sendApiError(res, 401, 'NRI_NO_TOKEN', 'Нет токена авторизации.');
    const code = String(req.params.code ?? '').trim().toUpperCase();
    const { lineId, fromStationId, toStationId, travelSeconds } = req.body as {
      lineId?: string;
  ...
  app.post('/neon_v1/services/nri/:code/map/metro/enter', async (req, res) => {
    ...
    try {
      ...
      if (!player) return sendApiError(res, 404, 'NRI_PLAYER_NOT_FOUND', 'Сначала создайте персонажа.');
      const station = await prisma.nriMetroStation.findFirst({
        where: { id: stationId.trim(), sessionId: session.id },
      });
      if (!station) return sendApiError(res, 404, 'NRI_METRO_STATION_NOT_FOUND', 'Станция не найдена.');
      const edges = await prisma.nriMetroEdge.findMany({ where: { sessionId: session.id } });
      const neighbors = neighborStationIds(station.id, edges as MetroEdge[]);
  ...
  app.post('/neon_v1/services/nri/:code/map/metro/ride', async (req, res) => {
    ...
    if (!auth) return sendApiError(res, 401, 'NRI_NO_TOKEN', 'Нет токена авторизации.');
    const code = String(req.params.code ?? '').trim().toUpperCase();
    const { toStationId } = req.body as { toStationId?: string };
    ...
    try {
      ...
      const metroStations: MetroStation[] = stations.map((s) => ({
        id: s.id,
        name: s.name,
        x: s.x,
        y: s.y,
      ...
      const path = findMetroPath(fromId, toStationId.trim(), metroStations, edges as MetroEdge[]);
      if (!path.ok) return sendApiError(res, 400, 'NRI_METRO_NO_PATH', path.error);

      const dest = stations.find((s) => s.id === toStationId.trim());
      if (!dest) return sendApiError(res, 404, 'NRI_METRO_STATION_NOT_FOUND', 'Станция не найдена.');
  ...
  app.post('/neon_v1/services/nri/:code/map/metro/ride/:rideId/complete', async (req, res) => {
    ...
    try {
      ...
      if (!ride) return sendApiError(res, 404, 'NRI_METRO_RIDE_NOT_FOUND', 'Поездка не найдена.');
      if (ride.completedAt) {
      ...
      const edges = await prisma.nriMetroEdge.findMany({ where: { sessionId: session.id } });
      const neighbors = neighborStationIds(dest.id, edges as MetroEdge[]);
      const neighborStations = await prisma.nriMetroStation.findMany({
        where: { id: { in: neighbors }, sessionId: session.id },
      });
      res.json({
  ...
  app.post('/neon_v1/services/nri/:code/map/metro/shops', async (req, res) => {
    ...
    if (!auth) return sendApiError(res, 401, 'NRI_NO_TOKEN', 'Нет токена авторизации.');
    const code = String(req.params.code ?? '').trim().toUpperCase();
    const { stationId, label, catalogIds } = req.body as {
      stationId?: string;
      label?: string;
      catalogIds?: string[];
    };
    ...
    try {
      ...
      if (!station) return sendApiError(res, 404, 'NRI_METRO_STATION_NOT_FOUND', 'Станция не найдена.');
      const ids =
        Array.isArray(catalogIds) && catalogIds.length
  ...
  app.post('/neon_v1/services/nri/:code/map/metro/shops/:shopId/buy', async (req, res) => {
    ...
    try {
      ...
      if (!shop) return sendApiError(res, 404, 'NRI_METRO_SHOP_NOT_FOUND', 'Лавка не найдена.');
      ...
      if (!cat) return sendApiError(res, 404, 'NRI_ITEM_NOT_FOUND', 'Предмет не найден в каталоге.');
      const item = catalogToServerInventoryItem(catalogId.trim());
      if (!item) return sendApiError(res, 500, 'NRI_ITEM_BUILD', 'Не удалось создать предмет.');
