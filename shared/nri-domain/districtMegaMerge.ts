...
import type { PlaceType } from './districtGrid.js';
...
export type MegaMergeType = (typeof MEGA_MERGE_TYPES)[number];
...
export type MegaShape = (typeof MEGA_SHAPES)[number];

export type MegaMergeInfo = {
  role: 'anchor' | 'cover';
  placeType: MegaMergeType;
  shape: MegaShape;
  anchorKey: string;
  span?: PlazaMegaSpan;
};

export function isMegaMergeType(t: string): t is MegaMergeType {
  return (MEGA_MERGE_TYPES as readonly string[]).includes(t);
}

export function parseMegaShape(s: string): MegaShape | null {
  return (MEGA_SHAPES as readonly string[]).includes(s) ? (s as MegaShape) : null;
}

/** Bounding box and occupied offsets relative to min row/col. */
export function shapeCellOffsets(shape: MegaShape): ReadonlyArray<readonly [number, number]> {
  switch (shape) {
    case '2x1':
      return [
        [0, 0],
...
function blockOffsets(n: number): Array<readonly [number, number]> {
  const out: Array<readonly [number, number]> = [];
  for (let r = 0; r < n; r++) for (let c = 0; c < n; c++) out.push([r, c]);
  return out;
}

export function shapeDims(shape: MegaShape): { rows: number; cols: number } {
  switch (shape) {
    case '2x1':
...
export function encodeMegaArtId(placeType: MegaMergeType, shape: MegaShape): string {
  return `mega:${placeType}:${shape}`;
}

export function encodeMegaCoverArtId(anchorZoneKey: string): string {
  return `mega_cover:${anchorZoneKey}`;
}

export function parseMegaArtId(
  artId: string | null | undefined
): { kind: 'anchor'; placeType: MegaMergeType; shape: MegaShape } | { kind: 'cover'; anchorKey: string } | null {
  ...
  if (artId.startsWith('mega:')) {
    ...
    if (!isMegaMergeType(placeType) || !shape) return null;
    return { kind: 'anchor', placeType, shape };
  }
  return null;
}

export type MegaCell = { gridRow: number; gridCol: number; placeType: string; zoneKey?: string };

export type ValidateMegaResult =
  | {
      ok: true;
      shape: MegaShape;
      placeType: MegaMergeType;
      originRow: number;
      originCol: number;
      /** Cells to apply (may be expanded to full bbox). */
      cells: MegaCell[];
    }
  | { ok: false; error: string };

function detectRectShape(h: number, w: number): MegaShape | null {
  if (h === 1 && w === 2) return '2x1';
  if (h === 2 && w === 1) return '1x2';
  if (h === 2 && w === 2) return '2x2';
  if (h === 3 && w === 3) return '3x3';
  if (h === 4 && w === 4) return '4x4';
  if (h === 6 && w === 6) return '6x6';
  return null;
}

function detectCornerOrCross(
  set: Set<string>,
  minR: number,
  minC: number,
  h: number,
  ...
  if (count === 3 && h === 2 && w === 2) {
    const has = (dr: number, dc: number) => set.has(`${minR + dr},${minC + dc}`);
    const nw = has(0, 0);
    const ne = has(0, 1);
    const sw = has(1, 0);
    const se = has(1, 1);
    if (nw && ne && sw && !se) return 'corner_se';
    if (nw && ne && se && !sw) return 'corner_sw';
    if (nw && sw && se && !ne) return 'corner_ne';
    if (ne && sw && se && !nw) return 'corner_nw';
  }
  if (count === 5 && h === 3 && w === 3) {
    const need = shapeCellOffsets('cross').map(([dr, dc]) => `${minR + dr},${minC + dc}`);
    if (need.every((k) => set.has(k)) && need.length === 5) return 'cross';
  }
  return null;
}

/** Majority placeType; prefers mergeable types on ties. */
export function majorityPlaceType(cells: MegaCell[]): string {
  ...
  for (const [t, n] of counts) {
    ...
    if (n === bestN) {
      if (isMegaMergeType(t) && !isMegaMergeType(best)) best = t;
    }
  }
  return best;
}

function pickMergeableType(cells: MegaCell[]): MegaMergeType | null {
  const maj = majorityPlaceType(cells);
  if (isMegaMergeType(maj)) return maj;
  const counts = new Map<string, number>();
  for (const c of cells) {
    if (!isMegaMergeType(c.placeType)) continue;
    counts.set(c.placeType, (counts.get(c.placeType) ?? 0) + 1);
  }
  let best: MegaMergeType | null = null;
  let bestN = 0;
  for (const [t, n] of counts) {
    if (n > bestN) {
      best = t as MegaMergeType;
      bestN = n;
    }
  }
  return best;
 ...
 */
export function validateMegaSelection(
  cells: MegaCell[],
  pool?: MegaCell[],
  opts?: { forcePlaceType?: string | null }
): ValidateMegaResult {
  if (cells.length < 2) return { ok: false, error: 'Выберите минимум 2 клетки.' };
  ...
  const resolveType = (resolved: MegaCell[]): MegaMergeType | null => {
  ...
  if (rect) {
    ...
    if (cells.length !== h * w && pool && pool.length) {
      const filled = pool.filter(
        (t) => t.gridRow >= minR && t.gridRow <= maxR && t.gridCol >= minC && t.gridCol <= maxC
      );
      if (filled.length === h * w) resolved = filled;
    }
...
export function assignShapeMegaRoles(
  originRow: number,
  originCol: number,
  shape: MegaShape
): Map<string, 'anchor' | 'cover'> {
  const offsets = shapeCellOffsets(shape);
  const out = new Map<string, 'anchor' | 'cover'>();
...
export function assignRectMegaRoles(
  originRow: number,
  originCol: number,
  shape: MegaShape
): Map<string, 'anchor' | 'cover'> {
...
export function buildExplicitMegaInfo(
  tiles: Array<{
    zoneKey: string;
    gridRow: number;
    gridCol: number;
