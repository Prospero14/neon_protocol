...
export type PlazaMegaRole = 'anchor' | 'cover';

export type PlazaMegaSpan = { x: number; y: number; w: number; h: number };

export type PlazaMegaInfo = {
  role: PlazaMegaRole;
  span?: PlazaMegaSpan;
};

export type BlockMegaKind =
  | 'plaza'
  | 'shack'
  | 'corp_hq'
  | 'corp_office'
...
export type BlockMegaInfo = PlazaMegaInfo & { kind: BlockMegaKind; size?: number };

export function blockShapeNxN(n: number): ReadonlyArray<readonly [number, number]> {
 ...
 */
export function assignBlockMegaRoles(
  byPos: Map<string, PlaceType>,
  placeType: PlaceType,
  size = 2
): Map<string, PlazaMegaRole> {
  const out = new Map<string, PlazaMegaRole>();
  ...
  for (const r of rowList) {
    for (const c of colList) {
      const k0 = posKey(r, c);
      if (claimed.has(k0)) continue;
      if (byPos.get(k0) !== placeType) continue;
      const cells = shape.map(([dr, dc]) => posKey(r + dr, c + dc));
      if (cells.some((k) => claimed.has(k) || byPos.get(k) !== placeType)) continue;
      out.set(cells[0]!, 'anchor');
      for (let i = 1; i < cells.length; i++) out.set(cells[i]!, 'cover');
...
export function assignPlazaMegaRoles(byPos: Map<string, PlaceType>): Map<string, PlazaMegaRole> {
 ...
 */
export function buildBlockMegaInfoMultiSize(
  tiles: Array<{
    gridRow: number;
    gridCol: number;
    x: number;
  ...
  for (const size of sorted) {
    ...
    for (const r of rowList) {
      for (const c of colList) {
        ...
        if (cellKeys.some((k) => claimed.has(k) || byPos.get(k) !== placeType)) continue;
        ...
        for (const k of cellKeys) claimed.add(k);
      }
    }
  }
  return out;
...
export function buildBlockMegaInfo(
  tiles: Array<{
    gridRow: number;
    gridCol: number;
    x: number;
...
export function buildPlazaMegaInfo(
  tiles: Array<{
    gridRow: number;
    gridCol: number;
    x: number;
...
): Map<string, PlazaMegaInfo> {
  const m = buildBlockMegaInfo(tiles, 'plaza', 'plaza', 2);
  const out = new Map<string, PlazaMegaInfo>();
  for (const [k, v] of m) out.set(k, { role: v.role, span: v.span });
  return out;
}

export function normalizePlazaLayout(grid: PlaceType[][]): number {
  ...
  let changed = 0;
  const demote: Array<[number, number]> = [];
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      if (grid[r]![c] !== 'plaza') continue;
      if (locked.has(posKey(r, c))) continue;
      const touch = [
        grid[r - 1]?.[c],
        grid[r + 1]?.[c],
        grid[r]?.[c - 1],
 ...
 */
export function normalizeParkingQuota(grid: PlaceType[][], style?: string): number {
  const rows = grid.length;
  const cols = grid[0]?.length ?? 0;
  let homes = 0;
  const parks: Array<{ r: number; c: number; score: number }> = [];
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      const t = grid[r]![c]!;
      if (t === 'house' || t === 'shack' || t === 'hotel' || t === 'service') homes++;
      if (t === 'parking') {
        let score = 0;
        for (const [dr, dc] of [
          [-1, 0],
          ...
          if (n === 'road' || n === 'bridge' || n === 'crossing') score += 3;
          if (n === 'shop' || n === 'electronics' || n === 'gunshop') score += 1;
        }
...
function canAbsorbToShack(t: PlaceType | undefined): boolean {
  return t === 'shack' || t === 'house' || t === 'generic' || t === 'secondhand';
}

export const SHACK_SINGLES_PER_MEGA = 6;

export function ensureShackMegaBlocks(grid: PlaceType[][], rng: () => number): number {
  const rows = grid.length;
  const cols = grid[0]?.length ?? 0;
  ...
  for (let r = 0; r < rows - 1; r++) {
    for (let c = 0; c < cols - 1; c++) {
      if (
        ...
        if (cells.some((k) => locked.has(k))) continue;
        for (const k of cells) locked.add(k);
        megas++;
      }
    }
  }
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      if (grid[r]![c] === 'shack') shackTotal++;
    }
  }
  if (shackTotal < 4) return 0;

  const singles = Math.max(0, shackTotal - megas * 4);
  let target = Math.max(megas, Math.ceil(singles / SHACK_SINGLES_PER_MEGA));
  if (shackTotal >= 8 && target < 1) target = 1;
  if (megas >= target) return 0;
  ...
  for (let r = 0; r < rows - 1; r++) {
    for (let c = 0; c < cols - 1; c++) {
      ...
      if (cells.some(([rr, cc]) => locked.has(posKey(rr, cc)))) continue;
      if (cells.some(([rr, cc]) => !canAbsorbToShack(grid[rr]![cc]))) continue;
      let score = 0;
      for (const [rr, cc] of cells) {
        if (grid[rr]![cc] === 'shack') score += 3;
        else if (grid[rr]![cc] === 'house') score += 1;
      }
      score += rng() * 0.5;
