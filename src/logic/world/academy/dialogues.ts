...
export const academy_dialogues: Record<string, DialogueTree> = {
  ...
  npc_professor_arkhipov: new DialogueBuilder('npc_professor_arkhipov')
    ...
    .addNode('welcome_friendly', 'ПРОФЕССОР АРХИПОВ', 'А, мой лучший адепт Чистого Кода! Вижу, твоя сигнатура стала еще стабильнее. Чем могу помочь сегодня?', [
      { text: 'Хочу освежить теорию.', nextId: 'cpu_lecture' },
      { text: '[Уйти]', nextId: 'LEAVE' }
    ])
    .addNode('welcome_repeat', 'ПРОФЕССОР АРХИПОВ', 'Рекурсия — мать учения. Возвращаешься к истокам? Ну что же, мой терминал всегда открыт для жаждущих знаний.', [
    ...
    .addNode('ram_lecture', 'ПРОФЕССОР АРХИПОВ', 'NEURAL_RAM — твоя память. Карта не срабатывает мгновенно, она загружается в RAM. Если память забита — ты не сможешь планировать новые действия.', [
      { text: 'А SYSTEM_STRESS?', nextId: 'stress_lecture' }
    ])
    .addNode('stress_lecture', 'ПРОФЕССОР АРХИПОВ', 'SYSTEM_STRESS — это перегрев. Если достигнет 100% — нейросеть уйдет в перезагрузку. Всегда следи за красной шкалой.', [
      { text: 'Как проходит OPERATIONS?', nextId: 'combat_lecture' }
    ])
    .addNode('combat_lecture', 'ПРОФЕССОР АРХИПОВ', ' OPERATIONS делятся на ARCHITECTURE (планирование) и ENGINEERING (исполнение). Никогда не начинай Engineering, если твоя архитектура — мусор.', [
